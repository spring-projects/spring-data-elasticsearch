...
public class RequestConverters {
	...
	public static Request sourceExists(GetRequest getRequest) {
		Request request = new Request(HttpMethod.HEAD.name(),
				endpoint(getRequest.index(), getRequest.id(), "_source"));
	...
	public static Request index(IndexRequest indexRequest) {
		...
		String endpoint = endpoint(indexRequest.index(), indexRequest.id(),
	...
	public static Request explain(ExplainRequest explainRequest) {
		Request request = new Request(HttpMethod.GET.name(), endpoint(explainRequest.index(), explainRequest.id(), "_explain"));
	...
	public static Request indexCreate(CreateIndexRequest createIndexRequest) {
		...
		request.setEntity(createEntity(new ToXContent() {
			@Override
	...
	static String endpoint(String index, String id, String endpoint) {
		return new EndpointBuilder().addPathPart(index, id).addPathPartAsIs(endpoint).build();
	}
