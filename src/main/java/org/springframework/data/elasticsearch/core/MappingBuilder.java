...
class MappingBuilder {
        ...
        if (FieldType.Auto != fieldAnnotation.type()) {
            ...
            if ((FieldType.Date == fieldAnnotation.type()) && (DateFormat.none != fieldAnnotation.format())) {
                xContentBuilder.field(
                        FIELD_FORMAT,
                        DateFormat.custom == fieldAnnotation.format() ? fieldAnnotation.pattern() : fieldAnnotation
                                .format());
            }
        }
    ...
    /**
     * Sort field for string type, for other types we don't need it(long, int, double, float) value of the field should
     * be converted to lowercase and not analise
     *
     * @throws IOException
     */
    private static void addSortMapping(XContentBuilder builder, java.lang.reflect.Field field, Field annotation)
            throws IOException {
        builder.startObject(FacetRequest.FIELD_SORT).field(FIELD_TYPE, TYPE_VALUE_STRING).field(FIELD_INDEX, "keyword")
